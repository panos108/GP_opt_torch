...
class MixedRBF(gpytorch.kernels.RBFKernel):
    def __init__(self, input_dim, variance=None, lengthscale=None, active_dims=None, integer_dims=None):
        super().__init__(ard_num_dims=input_dim, variance=variance, lengthscale=lengthscale, active_dims=active_dims)
        self.integer_dims = integer_dims

    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        x1[..., self.integer_dims] = torch.round(x1[..., self.integer_dims])
        if x2 is None:
            x2 = x1
        x2[..., self.integer_dims] = torch.round(x2[..., self.integer_dims])
        output = super().forward(x1, x2, diag)
        return output

class MixedMatern32(gpytorch.kernels.MaternKernel):
    def __init__(self, input_dim, variance=None, lengthscale=None, active_dims=None, integer_dims=None):
        super().__init__(nu=3 / 2, ard_num_dims=input_dim, variance=variance, lengthscale=lengthscale,
...
class MixedMatern52(gpytorch.kernels.MaternKernel):
    def __init__(self, input_dim, variance=None, lengthscale=None, active_dims=None, integer_dims=None):
        super().__init__(nu=5 / 2, ard_num_dims=input_dim, variance=variance, lengthscale=lengthscale,
                         active_dims=active_dims)
        self.integer_dims = integer_dims
