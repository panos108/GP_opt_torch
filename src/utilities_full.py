...
from src.extra_utils import MixedRBF, MixedMatern32, MixedMatern52, ConstrainedProblem
...
class BayesOpt(object):
    ...
    def solve(self, objective=None, xo=None, bounds=(0, 1), maxfun=20, N_initial=5,
        ...
        if N_constraints is None or N_constraints == 0:
            ...
            self.constraints = N_constraints
        ...
        self.card_of_funcs = 1 + self.constraints
        ...
        if self.X_initial is None:
            self.X = torch.from_numpy(sobol_seq.i4_sobol_generate(self.nx, N_initial)
    ...
    def predict_with_uncertainty(self,gp_model, x):
        """
        Predicts the mean and variance of the GP at input locations x.
        :param gp_model: The GP model
        :type gp_model: botorch.models.SingleTaskGP
        :param x: Input locations
        :type x: torch.Tensor
        :return: Mean and variance of the GP prediction
        :rtype: torch.Tensor, torch.Tensor
        """
        with torch.no_grad():
            # Get the predictive mean and variance from the GP model
    ...
    def acquisition_func(self, X_unscaled):
        ...
        gp = (self.gpmodel[0])
        mu, variance = self.predict_with_uncertainty(gp,x)
        if acquisition == 'Mean':
            ac = mu
    ...
    def gp_unscaled(self, x_np, idx, constr_prob=False):
        ...
        mu, variance = self.predict_with_uncertainty(self.gpmodel[idx],x_norm)

        mean = (mu * self.Y_std[idx] + self.Y_mean[idx]).item()
        var = (variance * (self.Y_std[idx])**2).item()
        if constr_prob:
